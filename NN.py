...
class MLP_NeuralNetwork(object):
    ...
    def __init__(self, input, hidden, output, iterations, learning_rate, momentum, rate_decay):
        ...
        self.wi = []
        for r in range(self.input):
            toadd = []
            for c in range(self.hidden):
                toadd.append( random.uniform(-.1, .1) )
            self.wi.append(toadd)
        self.wo = []
        for r in range(self.hidden):
            toadd = []
            for c in range(self.output):
                toadd.append( random.uniform(-.1, .1) )
            self.wo.append(toadd)

        #self.wi = np.random.uniform( low = -.1, high = .1, size = (self.input, self.hidden ) )
        #self.wo = np.random.uniform( low = -.1, high = .1, size = (self.hidden, self.output ) )
        ...
        for r in range(self.input):
            ...
            for c in range(self.hidden):
                toadd.append( 0)
            self.ci.append(toadd)
        self.co = []
        for r in range(self.hidden):
            toadd = []
            for c in range(self.output):
                toadd.append( 0 )
            self.co.append(toadd)
